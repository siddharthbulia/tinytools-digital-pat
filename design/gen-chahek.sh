#!/usr/bin/env bash
# REDO the "chahek" character from a new photo. Full sprite set (10 poses + 6 per-mood blinks),
# front-facing enforced. NOTE: caller clears characters/chahek + raw first so this regenerates fresh.
set -uo pipefail

KEY=$(grep -o 'sk-[A-Za-z0-9_-]*' ~/.gstack/openai.json | head -1)
OUT=/Users/siddharthbulia/Code/app-factory/apps/digital-pat/characters/chahek
RAW=~/.gstack/projects/digital-pat/people/chahek
PHOTO_SRC="/Users/siddharthbulia/.claude/image-cache/510989db-9381-4db2-9f1c-3ce84b976b88/16.png"
MAX=4
mkdir -p "$OUT" "$RAW"
...
BASE="Turn this person into an ADORABLE chibi pixel-art mascot, cute retro Tamagotchi style. $FRONT Huge round head about 60% of the body, big sparkly low-set eyes with white catchlights behind her glasses, and a warm, sweet, friendly smile. Tiny rounded body sitting upright, thick clean outline, soft warm palette, transparent background, single centered character. Keep her most recognizable features: shoulder-length wavy dark-brown/black hair, dark-framed eyeglasses, warm medium skin tone, a kind cheerful expression, and a black-and-white patterned (paisley/ornate) blouse."
PRE="Keep this EXACT chibi pixel character identical — same face, same glasses, same wavy hair, same FRONT-FACING forward pose, same proportions and pixel-art style, transparent background. Change ONLY: "
BLINKPRE="Keep this EXACT chibi pixel character identical — same outfit, same glasses, same hair, same front-facing pose, same everything. Change ONLY: both eyes gently closed in a soft blink behind the glasses. Transparent background, single centered character."
...
"meeting|wearing a smart dark blazer over the blouse, sitting up attentively"
...
wait
echo "DONE chahek: $(ls "$OUT" | wc -l | tr -d ' ') sprites"